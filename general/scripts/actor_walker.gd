...
func _ready():
	...
	if !is_facing_right:
		#Starts facing left, so make the sprite reflect this.
...
func _physics_process(delta):
	...
	if(is_on_floor()):
		if(position.x == position_x_last_frame):
			#We've bumped into a wall!
			flip()
		elif turns_at_ledges and !$AnimatedSprite2D/LedgeSensor.get_overlapping_bodies().size():
			flip()
		#Move
